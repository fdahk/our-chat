...
// 文件上传:把图片以 multipart/form-data 传到 /api/upload/single,返回可访问 URL。
@DependencyClient
    ...
    static let liveValue = UploadClient(
        uploadImage: { data, filename in
            ...
            let request = APIRequest(
                ...
                body: multipartBody(boundary: boundary, fieldName: "file", filename: filename, mimeType: "image/jpeg", data: data)
            )
