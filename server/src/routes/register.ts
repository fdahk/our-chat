...
router.post('/register', async (req, res) => {
  ...
  try {
    ...
    // 检查用户名是否已存在
    const [existingUsername] = await mySql.execute<RowDataPacket[]>(
    ...
    // 检查邮箱是否已存在
    const [existingEmail] = await mySql.execute<RowDataPacket[]>(
    ...
    // 如果提供了手机号，检查是否已存在
    if (phone) {
      const [existingPhone] = await mySql.execute<RowDataPacket[]>(
    ...
    // 获取刚创建的用户信息（不包含密码）
    const [newUser] = await mySql.execute<RowDataPacket[]>(
...
router.get('/check-phone', async (req, res) => {
  try {
    ...
    // 如果手机号为空，则设置为 undefined，配合数据库中的 phone 字段为 null
    if (phone?.trim() === '') {
      phone = undefined;
    }
    if (!phone) {
