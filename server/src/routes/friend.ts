...
router.get('/getFriendList/:id', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    const [friendId] = await mySql.query<RowDataPacket[]>(sql, [id]); // 获取好友id列表
    // 依据好友id列表获取好友信息
    let friendInfo: RowDataPacket[] = [];
    if (friendId.length > 0) {
      [friendInfo] = await mySql.query<RowDataPacket[]>(
        `SELECT id, username, avatar, gender FROM users WHERE id IN (?)`,
        [friendId.map((item) => item.friend_id)]
      );
    }
    // 合并好友id和好友信息
    let friendList: {
      friendId: Record<string, unknown>;
    ...
    if (friendId.length > 0) {
      friendList = {
        ...
        //注：用于用户单向好友管理，（备注、拉黑等
        friendId: friendId.reduce<Record<string, unknown>>((acc, item) => {
        ...
        // 注： 用于渲染好友信息
        friendInfo: friendInfo.reduce<Record<string, unknown>>((acc, item) => {
...
router.get('/searchUser', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    const [result] = await mySql.query<RowDataPacket[]>(sql, [keyword, keyword]);
    if (result.length === 0) {
      // 返回id，用于前端处理
      res.json({
        success: false,
    ...
    const [result2] = await mySql.query<RowDataPacket[]>(sql2, [userId, result[0].id]);
    if (result2.length > 0) {
      res.json({
        success: false,
        message: '已经是好友',
        data: { exist: true, isFriend: true, friendInfo: result[0] },
      });
      return;
    }
    // null,代表不是好友
...
router.put('/addFriend', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    res.json({
      success: true,
      message: '发起好友请求成功',
      data: { isFriend: false, friend_id: friend_id },
    });
  } catch (error) {
    console.log(error);
...
router.get('/getFriendReqs', authenticateToken, async (req, res) => {
  ...
  // 验证用户只能获取自己的好友请求
  if (req.user!.id.toString() !== (userId as string).toString()) {
    return res.status(403).json({
      success: false,
  ...
  try {
    ...
    const [rows] = await mySql.query<RowDataPacket[]>(sql, [userId]);
    const result = rows.reduce<Record<string, unknown>>((acc, item) => {
